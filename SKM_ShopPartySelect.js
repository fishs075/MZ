...
(() => {
    ...
    const EQUIP_PARAM_IDS = [2, 3, 4, 5, 6, 7];

    const isEquipItem = item => item && (DataManager.isWeapon(item) || DataManager.isArmor(item));

    function fallbackCurrentEquippedItem(actor, item) {
        if (!actor || !item || !item.etypeId) return null;
        const equips = actor.equips();
        const slots = actor.equipSlots();
        const candidates = [];
        for (let i = 0; i < slots.length; i++) {
            if (slots[i] === item.etypeId) {
                candidates.push(equips[i]);
            }
        }
        if (!candidates.length) return null;
        const paramId = DataManager.isWeapon(item) ? 2 : 3;
        let worstValue = Number.MAX_VALUE;
        let worstItem = null;
        for (const eq of candidates) {
            if (!eq) continue;
            const value = Number(eq.params?.[paramId] ?? 0);
            if (value < worstValue) {
                worstValue = value;
                worstItem = eq;
            }
    ...
    function fallbackParamsDiff(newItem, oldItem) {
        if (!newItem) return 0;
        return EQUIP_PARAM_IDS.reduce((acc, paramId) => {
            const newValue = Number(newItem.params?.[paramId] ?? 0);
            const oldValue = Number(oldItem?.params?.[paramId] ?? 0);
            return acc + (newValue - oldValue);
        }, 0);
    }

    function calculateEquipState(actor, item) {
        if (!actor || !isEquipItem(item)) return 'none';
        if (!actor.canEquip(item)) return 'cannot';
        const current = fallbackCurrentEquippedItem(actor, item);
        if (current && item && current.id === item.id) return 'equipped';
        const diff = fallbackParamsDiff(item, current);
        if (diff > 0) return 'up';
    ...
    class Window_ShopPartySelect extends Window_Selectable {
        ...
        currentShopItem() {
            if (this._statusWindow && this._statusWindow._item) {
                return this._statusWindow._item;
            }
            const scene = SceneManager._scene;
            if (!scene || !(scene instanceof Scene_Shop)) return null;
            const buyWindow = scene._buyWindow;
            const sellWindow = scene._sellWindow;
            const buyItem = buyWindow && typeof buyWindow.item === 'function' ? buyWindow.item() : null;
            const sellItem = sellWindow && typeof sellWindow.item === 'function' ? sellWindow.item() : null;
            if (buyWindow && buyWindow.active && buyWindow.visible) return buyItem;
            if (sellWindow && sellWindow.active && sellWindow.visible) return sellItem;
            if (buyWindow && buyWindow.visible) return buyItem;
            if (sellWindow && sellWindow.visible) return sellItem;
            return buyItem || sellItem || null;
        }

        getActorEquipState(actor) {
            if (!actor) return 'none';
            if (this._statusWindow && typeof this._statusWindow.getActorEquipState === 'function') {
                try {
                    const linkedState = this._statusWindow.getActorEquipState(actor);
                    if (linkedState !== undefined) return linkedState;
                } catch (_) {
                    // 連携先で例外が発生してもフォールバック評価を継続
                }
            }
            const item = this.currentShopItem();
            return calculateEquipState(actor, item);
        }
        ...
        drawItem(index) {
            ...
            // 装備状態バッジ（×/Ｅ/↑/↓）
            try {
                const state = this.getActorEquipState(actor);
                if (state && state !== 'none') {
